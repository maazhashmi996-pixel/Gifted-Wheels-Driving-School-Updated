...
const roadTestData = [
    ...
    {
        title: "Driving Test Practice",
        desc: "Driving Road Test Prep: Practice road- test maneuvers in our dual-brake vehicle with full support to build confidence for your DPS test. ",
            ...
            <div className="max-w-7xl mx-auto px-6 relative z-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                    {roadTestData.map((item, index) => (
                        ...
                        >
                            {/* Icon Container */}
                            <div className={`w-16 h-16 ${item.color} rounded-2xl flex items-center justify-center text-slate-700 mb-8 shadow-inner transition-transform duration-500`}>
                                {item.icon}
                            </div>
                            ...
                            {item.isSpecial && (
                                <Link href="/contact" className="text-yellow-600 font-bold text-xs uppercase tracking-widest hover:underline decoration-2 underline-offset-4">
                                    Contact for Pricing
                                </Link>
                            )}
                        </motion.div>
