...
export default function Navbar() {
    ...
    return (
        <nav className="fixed w-full z-50 top-0 bg-black/90 backdrop-blur-xl border-b border-white/10">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 h-24 flex items-center justify-between"> {/* Navbar height 20 se 24 kar di hai */}

                {/* Logo Section - Bigger Logo Update */}
                ...
                <ul className="hidden md:flex items-center space-x-8 lg:space-x-10 font-bold text-white/90">
                    {navLinks.map((item) => (
                        <li key={item.name}>
                            <Link
                                href={item.href}
                                className="hover:text-blue-400 transition-colors relative group text-sm uppercase tracking-wider"
                            >
                                {item.name}
                ...
                <div className="hidden md:block">
                    <Link
                        href="/contact"
                        className="bg-white text-black px-6 py-2.5 lg:px-8 lg:py-3 rounded-2xl font-black text-xs lg:text-sm uppercase tracking-widest hover:bg-blue-500 hover:text-white transition-all duration-300 active:scale-95 shadow-lg"
                    >
                        Get Started
                    </Link>
                </div>
